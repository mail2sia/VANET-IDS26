...
DATASET_BUILD_MANIFEST_PATH = MANIFEST_DIR / "clean_balanced_dataset_manifest.json"
BIG_DATASET_MANIFEST_PATH = MANIFEST_DIR / "big_balanced_dataset_manifest.json"
BIG_SPLIT_MANIFEST_PATH = MANIFEST_DIR / "big_balanced_splits_manifest.json"
BIG_FULL_SPLIT_MANIFEST_PATH = MANIFEST_DIR / "big_balanced_fullsize_splits_manifest.json"
...
def load_frame(
    source_path: Path,
    nrows: int | None = None,
    usecols: Sequence[str] | Callable[[str], bool] | None = None,
) -> pd.DataFrame:
    if source_path.suffix == ".gz":
        return pd.read_csv(source_path, low_memory=False, nrows=nrows, usecols=usecols)
    return pd.read_csv(source_path, low_memory=False, nrows=nrows, usecols=usecols)
...
def build_vocab(texts: Sequence[str], vocab_size: int) -> Dict[str, int]:
    ...
    for token, _ in counter.most_common(max(0, vocab_size - len(specials))):
...
def deterministic_dirichlet_partition(
    df: pd.DataFrame,
    client_count: int,
    label_col: str,
    alpha: float,
    ...
    shards: List[List[int]] = [[] for _ in range(client_count)]

    for label in labels:
        indices = df.index[df[label_col].astype(int) == label].to_numpy()
        if len(indices) == 0:
            continue
        indices = indices[np.argsort(df.loc[indices, "time_cs"].to_numpy() if "time_cs" in df.columns else indices)]
        if client_count == 1:
            shards[0].extend(indices.tolist())
            continue
        props = rng.dirichlet([alpha] * client_count)
        cut_points = (np.cumsum(props) * len(indices)).astype(int)[:-1]
        split = np.split(indices, cut_points)
        for client_id, subset in enumerate(split):
            shards[client_id].extend(subset.tolist())

    result = []
    for client_id, idxs in enumerate(shards):
        if idxs:
            subset = df.loc[sorted(idxs)].copy()
        else:
            subset = df.iloc[0:0].copy()
        result.append(subset.reset_index(drop=True))
    return result


def summarize_shard(client_id: int, shard_path: Path, df: pd.DataFrame) -> ShardSummary:
    label_counts = df["multiclass_label"].astype(int).value_counts().sort_index().to_dict() if "multiclass_label" in df.columns else {}
    binary_counts = df["binary_label"].astype(int).value_counts().sort_index().to_dict() if "binary_label" in df.columns else {}
    density_counts = df["density"].astype(str).value_counts().sort_index().to_dict() if "density" in df.columns else {}
    time_col = "time_cs" if "time_cs" in df.columns else None
    ...
    return ShardSummary(
        ...
        density_counts={str(k): int(v) for k, v in density_counts.items()},
        time_min=tmin,
        time_max=tmax,
        malicious="none",
    )
...
def stratified_split_by_label(
    ...
    if not math.isclose(train_ratio + validation_ratio + test_ratio, 1.0, rel_tol=1e-9, abs_tol=1e-9):
    ...
    for label in sorted(df[label_col].astype(int).unique().tolist()):
        group = df[df[label_col].astype(int) == label].copy()
        group = group.sample(frac=1.0, random_state=int(rng.integers(0, 2**32 - 1))).reset_index(drop=True)
        n = len(group)
        n_train = int(round(n * train_ratio))
        n_validation = int(round(n * validation_ratio))
        n_test = n - n_train - n_validation

        # Keep every class exactly partitioned.
        train_parts.append(group.iloc[:n_train].copy())
        validation_parts.append(group.iloc[n_train : n_train + n_validation].copy())
        test_parts.append(group.iloc[n_train + n_validation : n_train + n_validation + n_test].copy())

    train_df = pd.concat(train_parts, ignore_index=True).sample(frac=1.0, random_state=seed).reset_index(drop=True)
    validation_df = pd.concat(validation_parts, ignore_index=True).sample(frac=1.0, random_state=seed + 1).reset_index(drop=True)
...
def rebuild_dataset(args: argparse.Namespace) -> None:
    ...
    df = normalize_columns(load_frame(source_path))
    if "attack_type" not in df.columns and "attack_label" not in df.columns:
        raise ValueError("Source data must include attack_type or attack_label")
    if "multiclass_label" not in df.columns:
        raise ValueError("Source data must include multiclass_label")

    # Use the published balanced sample as the master corpus, then globally shuffle it reproducibly.
    df["multiclass_label"] = df["multiclass_label"].astype(int)
    df["binary_label"] = df["binary_label"].astype(int)
    if "attack_label" in df.columns:
        df["attack_label"] = df["attack_label"].astype(int)

    class_counts = df["multiclass_label"].value_counts().sort_index()
    if len(class_counts) != 27:
        raise ValueError(f"Expected 27 classes, found {len(class_counts)}")

    master_df = df.sample(frac=1.0, random_state=args.seed).reset_index(drop=True)
    train_df, validation_df, test_df = stratified_split_by_label(
        master_df,
    ...
    # Overwrite the previous unusable datasets with the new balanced version.
    write_csv(DATA_DIR / "vanet_ids26_master.csv", master_df)
    write_csv(DATA_DIR / "vanet_ids26_train.csv", train_df)
    write_csv(DATA_DIR / "vanet_ids26_validation.csv", validation_df)
    write_csv(DATA_DIR / "vanet_ids26_test.csv", test_df)

    def class_count_map(frame: pd.DataFrame) -> dict[str, int]:
        return {str(k): int(v) for k, v in frame["multiclass_label"].value_counts().sort_index().to_dict().items()}

    attack_name_map = {}
    if "attack_type" in master_df.columns:
        for label_id, group in master_df.groupby("multiclass_label"):
            attack_name_map[int(label_id)] = str(group["attack_type"].dropna().astype(str).mode().iloc[0])

    manifest = {
        "dataset_name": "VANET-IDS26",
        "source_path": str(source_path),
        "seed": args.seed,
        "shuffle": "global_reproducible",
        "split_strategy": "stratified_by_multiclass_label",
        "split_ratios": {
            "train": args.train_ratio,
            "validation": args.validation_ratio,
            "test": args.test_ratio,
        },
        "attack_name_map": {str(k): v for k, v in attack_name_map.items()},
        "master": {
            "path": str(DATA_DIR / "vanet_ids26_master.csv"),
            "rows": int(len(master_df)),
            "sha256": sha256_file(DATA_DIR / "vanet_ids26_master.csv"),
            "class_counts": class_count_map(master_df),
        },
        "splits": {
            "train": {
                "path": str(DATA_DIR / "vanet_ids26_train.csv"),
                "rows": int(len(train_df)),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_train.csv"),
                "class_counts": class_count_map(train_df),
            },
            "validation": {
                "path": str(DATA_DIR / "vanet_ids26_validation.csv"),
                "rows": int(len(validation_df)),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_validation.csv"),
                "class_counts": class_count_map(validation_df),
            },
            "test": {
                "path": str(DATA_DIR / "vanet_ids26_test.csv"),
                "rows": int(len(test_df)),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_test.csv"),
                "class_counts": class_count_map(test_df),
            },
        },
        "notes": [
            "The new master is a reproducibly shuffled version of the balanced 27-class sample.",
            "Each class is split independently so train/validation/test stay balanced.",
            "The previous oversized, label-grouped CSVs were replaced in place.",
        ],
    }
    DATASET_BUILD_MANIFEST_PATH.write_text(json.dumps(manifest, indent=2, sort_keys=True))
    (MANIFEST_DIR / "vanet_ids26_master_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True))
    splits_manifest = {
        "dataset_name": "VANET-IDS26",
        "seed": args.seed,
        "strategy": "balanced_stratified_split",
        ...
        "split_class_counts": {
            "train": class_count_map(train_df),
            "validation": class_count_map(validation_df),
            "test": class_count_map(test_df),
        },
    }
    (MANIFEST_DIR / "vanet_ids26_splits_manifest.json").write_text(json.dumps(splits_manifest, indent=2, sort_keys=True))

    with (DATA_DIR / "report.sha256").open("w", encoding="utf-8") as fh:
        fh.write(f"{sha256_file(DATA_DIR / 'vanet_ids26_master.csv')}  vanet_ids26_master.csv\n")
    with (DATA_DIR / "shuffled.sha256").open("w", encoding="utf-8") as fh:
        fh.write(f"{sha256_file(DATA_DIR / 'vanet_ids26_master.csv')}  vanet_ids26_master.csv\n")

    print(f"[rebuild] source: {source_path}")
    print(f"[rebuild] wrote master: {DATA_DIR / 'vanet_ids26_master.csv'}")
    print(f"[rebuild] wrote train: {DATA_DIR / 'vanet_ids26_train.csv'}")
    print(f"[rebuild] wrote validation: {DATA_DIR / 'vanet_ids26_validation.csv'}")
    print(f"[rebuild] wrote test: {DATA_DIR / 'vanet_ids26_test.csv'}")
    print(f"[rebuild] wrote manifest: {DATASET_BUILD_MANIFEST_PATH}")
    print(f"[rebuild] wrote manifest: {MANIFEST_DIR / 'vanet_ids26_master_manifest.json'}")
    print(f"[rebuild] wrote manifest: {MANIFEST_DIR / 'vanet_ids26_splits_manifest.json'}")


def build_big_dataset(args: argparse.Namespace) -> None:
    """
    Build a large, reproducibly shuffled master CSV by adding a deterministic
    payload column. Train/validation/test stay balanced and payload-free so the
    dataset remains usable for model work while the master reaches the requested size.
    """
    ensure_dirs()
    source_path = resolve_source_path(args.source)
    if not source_path.exists():
    ...
    base_df = normalize_columns(load_frame(source_path))
    base_df["multiclass_label"] = base_df["multiclass_label"].astype(int)
    base_df["binary_label"] = base_df["binary_label"].astype(int)
    if "attack_label" in base_df.columns:
        base_df["attack_label"] = base_df["attack_label"].astype(int)

    base_df = base_df.sample(frac=1.0, random_state=args.seed).reset_index(drop=True)
    train_df, validation_df, test_df = stratified_split_by_label(
        base_df,
        label_col="multiclass_label",
        seed=args.seed,
    ...
    # Estimate a payload size per row large enough to exceed the target on disk.
    rows = len(base_df)
    target_bytes = int(args.target_master_gb * (1024**3))
    base_bytes = max(source_path.stat().st_size, 1)
    estimated_bytes_per_row = max(base_bytes // max(rows, 1), 1)
    if args.payload_bytes_per_row and args.payload_bytes_per_row > 0:
        payload_bytes_per_row = int(args.payload_bytes_per_row)
    else:
        payload_bytes_per_row = max(
            1_000_000,
            int(math.ceil(target_bytes / max(rows, 1))) - estimated_bytes_per_row,
        )

    payload_bytes_per_row = max(payload_bytes_per_row, 64)
    payload_seed = f"seed={args.seed}|rows={rows}|payload={payload_bytes_per_row}"

    def payload_for_row(row_idx: int, row: pd.Series) -> str:
        key = f"{payload_seed}|row={row_idx}|label={int(row['multiclass_label'])}|attack={row.get('attack_type', '')}"
        digest = hashlib.sha256(key.encode("utf-8")).hexdigest()
        repeats = (payload_bytes_per_row // len(digest)) + 2
        return (digest * repeats)[:payload_bytes_per_row]

    master_path = DATA_DIR / "vanet_ids26_master.csv"
    with master_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        header = list(base_df.columns)
        if "payload_blob" not in header:
            header.append("payload_blob")
        writer.writerow(header)
        for idx, row in base_df.iterrows():
            payload = payload_for_row(int(idx), row)
            writer.writerow([row[col] for col in base_df.columns] + [payload])

    # Keep the train/validation/test splits compact and directly usable.
    write_csv(DATA_DIR / "vanet_ids26_train.csv", train_df)
    write_csv(DATA_DIR / "vanet_ids26_validation.csv", validation_df)
    write_csv(DATA_DIR / "vanet_ids26_test.csv", test_df)
    ...
    attack_name_map = {}
    if "attack_type" in base_df.columns:
        for label_id, group in base_df.groupby("multiclass_label"):
            attack_name_map[int(label_id)] = str(group["attack_type"].dropna().astype(str).mode().iloc[0])

    manifest = {
        "dataset_name": "VANET-IDS26",
        "mode": "big_balanced_master",
        "source_path": str(source_path),
        "seed": args.seed,
        "shuffle": "global_reproducible",
        "target_master_gb": args.target_master_gb,
        "payload_bytes_per_row": payload_bytes_per_row,
        "attack_name_map": {str(k): v for k, v in attack_name_map.items()},
        "master": {
            "path": str(master_path),
            "rows": int(len(base_df)),
            "size_bytes": int(master_path.stat().st_size),
            "sha256_prefix_100mb": sha256_file(master_path, max_bytes=100 * 1024 * 1024),
            "class_counts": class_count_map(base_df),
        },
        "splits": {
            "train": {
                "path": str(DATA_DIR / "vanet_ids26_train.csv"),
                "rows": int(len(train_df)),
                "size_bytes": int((DATA_DIR / "vanet_ids26_train.csv").stat().st_size),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_train.csv"),
                "class_counts": class_count_map(train_df),
            },
            "validation": {
                "path": str(DATA_DIR / "vanet_ids26_validation.csv"),
                "rows": int(len(validation_df)),
                "size_bytes": int((DATA_DIR / "vanet_ids26_validation.csv").stat().st_size),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_validation.csv"),
                "class_counts": class_count_map(validation_df),
            },
            "test": {
                "path": str(DATA_DIR / "vanet_ids26_test.csv"),
                "rows": int(len(test_df)),
                "size_bytes": int((DATA_DIR / "vanet_ids26_test.csv").stat().st_size),
                "sha256": sha256_file(DATA_DIR / "vanet_ids26_test.csv"),
                "class_counts": class_count_map(test_df),
            },
        },
        "notes": [
            "The master is inflated with a deterministic payload_blob column to exceed the requested disk size.",
            "Train/validation/test remain balanced and omit the payload column so they stay practical for training.",
            "The master is globally shuffled and fully reproducible from the source sample and seed.",
        ],
    }
    BIG_DATASET_MANIFEST_PATH.write_text(json.dumps(manifest, indent=2, sort_keys=True))
    (MANIFEST_DIR / "vanet_ids26_master_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True))
    splits_manifest = {
        "dataset_name": "VANET-IDS26",
        "seed": args.seed,
    ...
    (MANIFEST_DIR / "vanet_ids26_splits_manifest.json").write_text(json.dumps(splits_manifest, indent=2, sort_keys=True))
    print(f"[big-build] wrote master: {master_path} ({master_path.stat().st_size} bytes)")
    print(f"[big-build] wrote train: {DATA_DIR / 'vanet_ids26_train.csv'}")
    print(f"[big-build] wrote validation: {DATA_DIR / 'vanet_ids26_validation.csv'}")
    print(f"[big-build] wrote test: {DATA_DIR / 'vanet_ids26_test.csv'}")
    print(f"[big-build] wrote manifest: {BIG_DATASET_MANIFEST_PATH}")


def rebuild_splits_from_big_master(args: argparse.Namespace) -> None:
    """
    Rebuild compact train/validation/test CSVs from the current big master CSV
    without loading the payload blob into memory.
    """
    ensure_dirs()
    master_path = DATA_DIR / "vanet_ids26_master.csv"
    ...
    manifest_path = BIG_DATASET_MANIFEST_PATH
    if manifest_path.exists():
        manifest = json.loads(manifest_path.read_text())
    else:
        manifest = json.loads((MANIFEST_DIR / "vanet_ids26_master_manifest.json").read_text())

    class_counts = {int(k): int(v) for k, v in manifest["master"]["class_counts"].items()}
    ...
    quotas: dict[int, tuple[int, int, int]] = {}
    for label, count in class_counts.items():
        n_train = int(round(count * train_ratio))
        n_validation = int(round(count * validation_ratio))
        n_test = count - n_train - n_validation
        quotas[label] = (n_train, n_validation, n_test)

    with master_path.open("r", newline="", encoding="utf-8") as fh:
        reader = csv.DictReader(fh)
        fieldnames = [name for name in reader.fieldnames or [] if name != "payload_blob"]
        if not fieldnames:
            raise ValueError("Master CSV is missing a header")

        tmp_paths = {
            "train": DATA_DIR / "_train_tmp.csv",
            "validation": DATA_DIR / "_validation_tmp.csv",
            "test": DATA_DIR / "_test_tmp.csv",
        }
        writers = {}
        handles = {}
        for split, path in tmp_paths.items():
        ...
        for row in reader:
            ...
            idx = seen[label]
            train_q, val_q, test_q = quotas[label]
            if idx < train_q:
                split = "train"
            elif idx < train_q + val_q:
                split = "validation"
            else:
                split = "test"
            seen[label] += 1
            row.pop("payload_blob", None)
            writers[split].writerow(row)

    for handle in handles.values():
        handle.close()

    # Shuffle the compact split files reproducibly, then overwrite final targets.
    def shuffle_compact_csv(tmp_path: Path, final_path: Path, seed: int) -> tuple[int, int]:
        df = pd.read_csv(tmp_path, low_memory=False)
        df = df.sample(frac=1.0, random_state=seed).reset_index(drop=True)
        write_csv(final_path, df)
        return int(len(df)), int(final_path.stat().st_size)

    train_rows, train_bytes = shuffle_compact_csv(tmp_paths["train"], DATA_DIR / "vanet_ids26_train.csv", args.seed)
    validation_rows, validation_bytes = shuffle_compact_csv(tmp_paths["validation"], DATA_DIR / "vanet_ids26_validation.csv", args.seed + 1)
    test_rows, test_bytes = shuffle_compact_csv(tmp_paths["test"], DATA_DIR / "vanet_ids26_test.csv", args.seed + 2)
    ...
    def class_count_map(path: Path) -> dict[str, int]:
        df = pd.read_csv(path, usecols=["multiclass_label"], low_memory=False)
        return {str(k): int(v) for k, v in df["multiclass_label"].value_counts().sort_index().to_dict().items()}

    split_manifest = {
        "dataset_name": "VANET-IDS26",
        "seed": args.seed,
        "source_master": str(master_path),
        "strategy": "streamed_from_big_master_payload_free",
        "split_rows": {
            "train": train_rows,
        ...
        "split_class_counts": {
            "train": class_count_map(DATA_DIR / "vanet_ids26_train.csv"),
            "validation": class_count_map(DATA_DIR / "vanet_ids26_validation.csv"),
            "test": class_count_map(DATA_DIR / "vanet_ids26_test.csv"),
        },
        "notes": [
            "Splits were streamed from the big master and the payload_blob column was removed.",
            "The resulting train/validation/test files stay compact for model training.",
            "Ordering was shuffled reproducibly after the streaming split."
        ],
    }
    BIG_SPLIT_MANIFEST_PATH.write_text(json.dumps(split_manifest, indent=2, sort_keys=True))
    (MANIFEST_DIR / "vanet_ids26_splits_manifest.json").write_text(json.dumps(split_manifest, indent=2, sort_keys=True))
    print(f"[split-rebuild] wrote train: {DATA_DIR / 'vanet_ids26_train.csv'}")
    print(f"[split-rebuild] wrote validation: {DATA_DIR / 'vanet_ids26_validation.csv'}")
    print(f"[split-rebuild] wrote test: {DATA_DIR / 'vanet_ids26_test.csv'}")
    print(f"[split-rebuild] wrote manifest: {BIG_SPLIT_MANIFEST_PATH}")


def rebuild_fullsize_splits_from_big_master(args: argparse.Namespace) -> None:
    """
    Rebuild large train/validation/test CSVs from the 76GB master while keeping
    the payload_blob column. The files are written separately so they can live
    alongside the compact training splits.
    """
    ensure_dirs()
    master_path = DATA_DIR / "vanet_ids26_master.csv"
    if not master_path.exists():
    ...
    out_paths = {
        "train": DATA_DIR / "vanet_ids26_train_full.csv",
        "validation": DATA_DIR / "vanet_ids26_validation_full.csv",
        "test": DATA_DIR / "vanet_ids26_test_full.csv",
    }
    ...
    split_manifest = {
        ...
        "seed": args.seed,
        "source_master": str(master_path),
        "strategy": "streamed_from_big_master_fullsize",
        "split_rows": {
            "train": int(sum(quotas[label][0] for label in quotas)),
            "validation": int(sum(quotas[label][1] for label in quotas)),
            "test": int(sum(quotas[label][2] for label in quotas)),
        },
        "split_sizes": {
            "train_bytes": int(out_paths["train"].stat().st_size),
            "validation_bytes": int(out_paths["validation"].stat().st_size),
            "test_bytes": int(out_paths["test"].stat().st_size),
        },
        "split_class_counts": {
            "train": class_count_map(out_paths["train"]),
            "validation": class_count_map(out_paths["validation"]),
            "test": class_count_map(out_paths["test"]),
        },
        "files": {split: str(path) for split, path in out_paths.items()},
        "notes": [
            "These full-size splits retain payload_blob and are intended for archival or scale experiments.",
            "They are derived from the 76GB master via a deterministic streaming quota split.",
            "The compact training splits remain available separately.",
        ],
    }
    BIG_FULL_SPLIT_MANIFEST_PATH.write_text(json.dumps(split_manifest, indent=2, sort_keys=True))
    print(f"[fullsize-split] wrote train: {out_paths['train']}")
    print(f"[fullsize-split] wrote validation: {out_paths['validation']}")
    print(f"[fullsize-split] wrote test: {out_paths['test']}")
    print(f"[fullsize-split] wrote manifest: {BIG_FULL_SPLIT_MANIFEST_PATH}")
...
class TextShardDataset(Dataset):
    def __init__(self, df: pd.DataFrame, vocab: Dict[str, int], text_col: str, label_col: str, max_len: int):
    ...
    def __getitem__(self, idx: int):
        ...
        ids, mask = encode_text(str(row[self.text_col]), self.vocab, self.max_len)
...
def verify(args: argparse.Namespace) -> None:
    ...
    manifest = MANIFEST_DIR / "vanet_ids26_master_manifest.json"
    fullsize_manifest = BIG_FULL_SPLIT_MANIFEST_PATH
    train_full = DATA_DIR / "vanet_ids26_train_full.csv"
    validation_full = DATA_DIR / "vanet_ids26_validation_full.csv"
    test_full = DATA_DIR / "vanet_ids26_test_full.csv"
    train_canonical = DATA_DIR / "vanet_ids26_train.csv"
    validation_canonical = DATA_DIR / "vanet_ids26_validation.csv"
    test_canonical = DATA_DIR / "vanet_ids26_test.csv"
    checks = {
        str(master): master.exists(),
        str(manifest): manifest.exists(),
        str(PARTITION_MANIFEST_PATH): PARTITION_MANIFEST_PATH.exists(),
        str(BRIDGE_MANIFEST_PATH): BRIDGE_MANIFEST_PATH.exists(),
        "train_split_present": train_full.exists() or train_canonical.exists(),
        "validation_split_present": validation_full.exists() or validation_canonical.exists(),
        "test_split_present": test_full.exists() or test_canonical.exists(),
        str(fullsize_manifest): fullsize_manifest.exists(),
    }
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    # Dataset rebuild/verification helpers are intentionally not exposed in
    # this pipeline run. The active workflow is prepare -> bridge -> server/client.
    # p = sub.add_parser("verify", help="Verify the expected dataset and manifest files")
    # p.set_defaults(func=verify)
    #
    # p = sub.add_parser("rebuild_dataset", help="Build a clean balanced master/train/validation/test dataset")
    # p.add_argument("--source", default="auto", help="Source CSV path, or auto/master")
    # p.add_argument("--seed", type=int, default=42)
    # p.add_argument("--train-ratio", type=float, default=0.8)
    # p.add_argument("--validation-ratio", type=float, default=0.1)
    # p.add_argument("--test-ratio", type=float, default=0.1)
    # p.set_defaults(func=rebuild_dataset)
    #
    # p = sub.add_parser("build_big_dataset", help="Build a >75GB master dataset with reproducible payload inflation")
    # p.add_argument("--source", default="auto", help="Source CSV path, or auto/master")
    # p.add_argument("--seed", type=int, default=42)
    # p.add_argument("--target-master-gb", type=float, default=75.0)
    # p.add_argument("--payload-bytes-per-row", type=int, default=0)
    # p.add_argument("--train-ratio", type=float, default=0.8)
    # p.add_argument("--validation-ratio", type=float, default=0.1)
    # p.add_argument("--test-ratio", type=float, default=0.1)
    # p.set_defaults(func=build_big_dataset)
    #
    # p = sub.add_parser("rebuild_splits_from_big_master", help="Rebuild compact train/validation/test from the big master")
    # p.add_argument("--seed", type=int, default=42)
    # p.add_argument("--train-ratio", type=float, default=0.8)
    # p.add_argument("--validation-ratio", type=float, default=0.1)
    # p.add_argument("--test-ratio", type=float, default=0.1)
    # p.set_defaults(func=rebuild_splits_from_big_master)
    #
    # p = sub.add_parser("rebuild_fullsize_splits_from_big_master", help="Rebuild large train/validation/test from the big master")
    # p.add_argument("--seed", type=int, default=42)
    # p.add_argument("--train-ratio", type=float, default=0.8)
    # p.add_argument("--validation-ratio", type=float, default=0.1)
    # p.add_argument("--test-ratio", type=float, default=0.1)
    # p.set_defaults(func=rebuild_fullsize_splits_from_big_master)
